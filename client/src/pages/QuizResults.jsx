    ...
    <div className="max-w-3xl mx-auto p-4">
      <div className="bg-white rounded-lg shadow-md p-6 mb-8">
        ...
        <div className="flex items-center text-sm text-gray-600 mb-6">
          <span className="mr-4">Lần thử #{result.attemptNumber || 1}</span>
          <span>{new Date(result.completedAt).toLocaleString()}</span>
        </div>

        <div className={`p-6 rounded-lg mb-8 ${percentageScore >= 60 ? 'bg-green-50 border border-green-200' : 'bg-red-50 border border-red-200'}`}>
          <div className="text-center">
            <h2 className="text-xl font-semibold mb-2">
              Điểm số của bạn: {result.correctAnswers || 0} / {result.totalQuestions || 0}
            </h2>
            <div className="text-3xl font-bold mb-2">
              {percentageScore.toFixed(1)}%
            </div>
            <p className={percentageScore >= 60 ? 'text-green-700' : 'text-red-700'}>
              {percentageScore >= 60 ? 'Chúc mừng! Bạn đã vượt qua bài kiểm tra!' : 'Cố gắng luyện tập thêm nhé!'}
            </p>
          </div>
        </div>
